...
Given the rate of increase of a sphere's surface area, at what rate does its volume grow? How rapidly does the water level rise in a conical funnel? When measuring the height of a structure by observing an angle, how does a small angular error propagate into the computed height? This lesson addresses these questions using a unified three-step procedure: relate the variables, differentiate, and solve. The generality of this method is demonstrated through its applicability to a wide range of problems.

::: {.callout-tip collapse="true"}
## Motivation

Related rates determine how fast one quantity is changing given the rate of change of a different, connected quantity:

- **Medical imaging**: as a spherical tumor grows, physicians must determine how fast the volume increases based on surface area measurements from imaging scans
- **Engineering**: filling a conical storage tank with material -- engineers must predict how quickly the level rises to prevent overflow
- **Architectural design**: when modifying a window while keeping the total framing material constant, related rates determine how changing one dimension forces the other to adjust
- **Surveying**: measuring the height of a tall structure from the ground using angles -- small measurement errors in the angle propagate into errors in the computed height
...
Every variable that depends on time acquires a $\frac{d(\cdot)}{dt}$ factor via the chain rule. Constants remain unchanged.
:::
...
This arises frequently in related rates. For example, differentiating an area $A = 2rh$ where both $r$ and $h$ change over time gives:
...
These formulas need not be memorized at this stage -- they will be derived later in the course using integration.
:::
...
**Animated demonstration -- expanding sphere cross-section showing dV/dr:**

```{=html}
<div class="d3-container" id="sphere_expand_1020">
  <svg id="sphere_expand_svg_1020" width="100%" height="420"></svg>
  <div class="d3-controls">
    <button id="sphere_expand_play_1020">&#9654; Play</button>
    <label>dA/dt: <input type="range" id="sphere_expand_dadt_1020" min="1" max="15" step="0.5" value="7">
    <span id="sphere_expand_dadt_val_1020">7.0</span> m&sup2;/min</label>
  </div>
</div>
<script>
(function(){
  var svg = d3.select("#sphere_expand_svg_1020");
  var width = 600, height = 420;
  svg.attr("viewBox", "0 0 " + width + " " + height);

  var cx = width / 2 - 60, cy = height / 2 - 20;
  var r = 20, maxR = 140, minR = 15;
  var dAdt = 7, playing = false, animId = null;

  var outerRing = svg.append("circle").attr("cx", cx).attr("cy", cy)
    .attr("fill", "none").attr("stroke", "#c74440").attr("stroke-width", 2).attr("stroke-dasharray", "6,3").attr("opacity", 0.5);
  var sphere = svg.append("circle").attr("cx", cx).attr("cy", cy)
    .attr("fill", "rgba(45,112,179,0.12)").attr("stroke", "#2d70b3").attr("stroke-width", 2.5);
  var rLine = svg.append("line").attr("x1", cx).attr("y1", cy).attr("stroke", "#c74440").attr("stroke-width", 2);
  var drLine = svg.append("line").attr("stroke", "#388c46").attr("stroke-width", 3);

  var ellipseH = svg.append("ellipse").attr("cx", cx).attr("cy", cy)
    .attr("fill", "none").attr("stroke", "#2d70b3").attr("stroke-width", 1).attr("stroke-dasharray", "3,3").attr("opacity", 0.5);

  var rLabel = svg.append("text").attr("font-size", "14px").attr("fill", "#c74440").attr("text-anchor", "middle");
  var info1 = svg.append("text").attr("x", width / 2 + 40).attr("y", height - 55).attr("text-anchor", "middle").attr("font-size", "14px").attr("fill", "#333");
  var info2 = svg.append("text").attr("x", width / 2 + 40).attr("y", height - 33).attr("text-anchor", "middle").attr("font-size", "14px").attr("fill", "#2d70b3");
  var info3 = svg.append("text").attr("x", width / 2 + 40).attr("y", height - 11).attr("text-anchor", "middle").attr("font-size", "14px").attr("fill", "#388c46");

  var barX = width - 110, barW = 30, barMaxH = 250, barY0 = cy + maxR;
  svg.append("rect").attr("x", barX).attr("y", barY0 - barMaxH).attr("width", barW).attr("height", barMaxH)
    .attr("fill", "none").attr("stroke", "#aaa").attr("stroke-width", 1);
  svg.append("text").attr("x", barX + barW / 2).attr("y", barY0 + 18).attr("text-anchor", "middle").attr("font-size", "11px").attr("fill", "#555").text("dV/dt");
  var bar = svg.append("rect").attr("x", barX).attr("width", barW).attr("fill", "#388c46").attr("opacity", 0.6);
  var barLabel = svg.append("text").attr("x", barX + barW / 2).attr("text-anchor", "middle").attr("font-size", "12px").attr("fill", "#388c46");

  function update() {
    var rReal = r / maxR * 10;
    sphere.attr("r", r);
    ellipseH.attr("rx", r).attr("ry", r * 0.3);
    outerRing.attr("r", r + 5);
    rLine.attr("x2", cx + r).attr("y2", cy);
    drLine.attr("x1", cx + r).attr("y1", cy - 3).attr("x2", cx + r + 5).attr("y2", cy - 3);
    rLabel.attr("x", cx + r / 2).attr("y", cy - 8).text("r=" + rReal.toFixed(1));

    var drdt = dAdt / (8 * Math.PI * rReal);
    var dVdt = dAdt * rReal / 2;
    var A = 4 * Math.PI * rReal * rReal;
    var V = 4 / 3 * Math.PI * rReal * rReal * rReal;

    info1.text("A=4\u03C0r\u00B2=" + A.toFixed(1) + " | V=(4/3)\u03C0r\u00B3=" + V.toFixed(1));
    info2.text("dA/dt=" + dAdt.toFixed(1) + " \u2192 dr/dt=" + drdt.toFixed(3));
    info3.text("dV/dt = (dA/dt)\u00B7r/2 = " + dVdt.toFixed(1) + " m\u00B3/min");

    var barH = Math.min(dVdt / 40 * barMaxH, barMaxH);
    bar.attr("y", barY0 - barH).attr("height", barH);
    barLabel.attr("x", barX + barW / 2).attr("y", barY0 - barH - 5).text(dVdt.toFixed(1));
  }
  ...
  document.getElementById("sphere_expand_play_1020").addEventListener("click", function() {
    playing = !playing;
  ...
  document.getElementById("sphere_expand_dadt_1020").addEventListener("input", function() {
    dAdt = parseFloat(this.value);
    document.getElementById("sphere_expand_dadt_val_1020").textContent = dAdt.toFixed(1);
    update();
  });
...
**Interactive demonstration -- dependence of $\frac{dV}{dt}$ on the radius when $\frac{dA}{dt} = 7$:**
...
**Key observation.** Instead of computing the frustum volume, consider the **empty cone at the top**. Let $y$ be the height of the empty cone and $r$ its base radius:
...
**Interactive demonstration -- the conical container cross-section as water fills:**
...
**Animated demonstration -- conical tank filling with water:**

```{=html}
<div class="d3-container" id="cone_drain_1020">
  <svg id="cone_drain_svg_1020" width="100%" height="420"></svg>
  <div class="d3-controls">
    <button id="cone_drain_play_1020">&#9654; Play</button>
    <label>Flow rate dV/dt: <input type="range" id="cone_drain_flow_1020" min="0.5" max="5" step="0.1" value="2">
    <span id="cone_drain_flow_val_1020">2.0</span> cm&sup3;/s</label>
  </div>
</div>
<script>
(function(){
  var svg = d3.select("#cone_drain_svg_1020");
  var width = 600, height = 420;
  svg.attr("viewBox", "0 0 " + width + " " + height);

  var coneH = 280, coneTopW = 180;
  var cx = 220, tipY = 340, topY = tipY - coneH;
  var flow = 2, playing = false, animId = null;
  var waterH = 0.1;
  var H = 6, R = 4;

  svg.append("line").attr("x1", cx).attr("y1", tipY).attr("x2", cx - coneTopW / 2).attr("y2", topY)
    .attr("stroke", "#aaa").attr("stroke-width", 2);
  svg.append("line").attr("x1", cx).attr("y1", tipY).attr("x2", cx + coneTopW / 2).attr("y2", topY)
    .attr("stroke", "#aaa").attr("stroke-width", 2);
  svg.append("line").attr("x1", cx - coneTopW / 2).attr("y1", topY).attr("x2", cx + coneTopW / 2).attr("y2", topY)
    .attr("stroke", "#aaa").attr("stroke-width", 2);

  var waterPoly = svg.append("polygon").attr("fill", "rgba(45,112,179,0.25)").attr("stroke", "#2d70b3").attr("stroke-width", 2);
  var hLine = svg.append("line").attr("stroke", "#c74440").attr("stroke-width", 2).attr("stroke-dasharray", "5,3");
  var rLine = svg.append("line").attr("stroke", "#388c46").attr("stroke-width", 2).attr("stroke-dasharray", "5,3");
  var hLabel = svg.append("text").attr("font-size", "13px").attr("fill", "#c74440");
  var rLabel = svg.append("text").attr("font-size", "13px").attr("fill", "#388c46");

  var info1 = svg.append("text").attr("x", width / 2 + 60).attr("y", height - 55).attr("text-anchor", "middle").attr("font-size", "13px").attr("fill", "#333");
  var info2 = svg.append("text").attr("x", width / 2 + 60).attr("y", height - 33).attr("text-anchor", "middle").attr("font-size", "13px").attr("fill", "#2d70b3");
  var info3 = svg.append("text").attr("x", width / 2 + 60).attr("y", height - 11).attr("text-anchor", "middle").attr("font-size", "14px").attr("fill", "#c74440");

  var dhBar = svg.append("rect").attr("x", width - 100).attr("width", 25).attr("fill", "#c74440").attr("opacity", 0.6);
  svg.append("text").attr("x", width - 87).attr("y", tipY + 18).attr("text-anchor", "middle").attr("font-size", "11px").text("dh/dt");
  var dhLabel = svg.append("text").attr("x", width - 87).attr("text-anchor", "middle").attr("font-size", "12px").attr("fill", "#c74440");

  function update() {
    var frac = waterH / H;
    var pixH = frac * coneH;
    var pixW = frac * coneTopW / 2;
    var wBot = tipY;
    var wTop = tipY - pixH;

    waterPoly.attr("points",
      cx + "," + wBot + " " +
      (cx - pixW) + "," + wTop + " " +
      (cx + pixW) + "," + wTop);

    hLine.attr("x1", cx + pixW + 10).attr("y1", tipY).attr("x2", cx + pixW + 10).attr("y2", wTop);
    hLabel.attr("x", cx + pixW + 20).attr("y", (tipY + wTop) / 2 + 4).text("h=" + waterH.toFixed(2));

    var rWater = (2 / 3) * waterH;
    rLine.attr("x1", cx).attr("y1", wTop).attr("x2", cx + pixW).attr("y2", wTop);
    rLabel.attr("x", cx + pixW / 2).attr("y", wTop - 6).text("r=" + rWater.toFixed(2));

    var V = (4 * Math.PI / 27) * waterH * waterH * waterH;
    var dhdt = flow / (4 * Math.PI / 9 * waterH * waterH);
    if (!isFinite(dhdt)) dhdt = 999;

    info1.text("Similar triangles: r/h = R/H = 2/3 \u2192 r = (2/3)h");
    info2.text("V = (4\u03C0/27)h\u00B3 = " + V.toFixed(2) + " cm\u00B3");
    info3.text("dh/dt = " + (dhdt > 50 ? ">50" : dhdt.toFixed(2)) + " cm/s  (faster when h is small!)");

    var barH = Math.min(dhdt * 30, 250);
    dhBar.attr("y", tipY - barH).attr("height", barH);
    dhLabel.attr("y", tipY - barH - 5).text(dhdt > 50 ? ">50" : dhdt.toFixed(2));
  }
  ...
  function animate() {
    ...
    var dhdt = flow / (4 * Math.PI / 9 * waterH * waterH);
    waterH += dhdt * 0.016;
    if (waterH > H - 0.1) { waterH = 0.1; }
    if (waterH < 0.1) waterH = 0.1;
    update();
    animId = requestAnimationFrame(animate);
  }

  document.getElementById("cone_drain_play_1020").addEventListener("click", function() {
    playing = !playing;
    this.textContent = playing ? "\u23F8 Pause" : "\u25B6 Play";
    if (playing) animate();
    else if (animId) cancelAnimationFrame(animId);
  ...
  document.getElementById("cone_drain_flow_1020").addEventListener("input", function() {
    flow = parseFloat(this.value);
    document.getElementById("cone_drain_flow_val_1020").textContent = flow.toFixed(1);
    update();
  });
...
**Substitute the known values.** At $\theta = 45°$:
...
Propagated error is related rates without the time variable. Instead of $\frac{dh}{dt}$ and $\frac{d\theta}{dt}$, one works directly with differentials $dh$ and $d\theta$. The key observation: one should **not** compute $h(45.1°) - h(45°)$ directly. Instead, treating the small deviation as a differential is more efficient and yields a formula valid for any angle.
:::

**Interactive demonstration -- dependence of the height error $dh$ on the measurement angle $\theta$:**
