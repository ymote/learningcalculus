---
title: "Related Rates via the Physics (Vector Projection) Method"
date: 2025-10-16
---

This lesson presents the vector projection method for solving related rates problems involving rigid objects such as rods and beams -- a technique that determines velocities without explicit differentiation. We also prove the Law of Sines using the circumscribed circle, establishing one of the fundamental results in triangle geometry.

::: {.callout-tip collapse="true"}
## Motivation

Understanding how the velocities of connected objects relate to each other arises in numerous engineering and physical applications:

- **Robotics**: a robotic arm consists of rigid links connected at joints -- engineers use velocity projections to determine how fast the end effector moves when a motor actuates one joint
- **Mechanical engineering**: pistons and crankshafts are connected by rigid rods, and the piston speed is determined by projecting the crankshaft's velocity along the rod direction
- **Biomechanics**: when a bat swings, every point on the bat moves at a different speed -- the rigid-body constraint determines the tip velocity relative to the handle
- **Structural engineering**: as a crane arm rotates, the cable attachment point moves along a circle and the load swings -- predicting the load's velocity requires decomposing motion along and perpendicular to the cable
- **Computational physics**: physics engines simulate rigid bodies by enforcing constraints on the motion of connected parts, using the projection methods developed in this lesson
:::

## Topics Covered

- Related rates for rigid bodies using the **vector projection method** (no derivatives needed)
- Velocity decomposition into **radial** and **tangential** components along a rigid rod
- Rigid body constraint: radial components of velocity at both ends must be equal
- The result $v = u \dfrac{\cos\theta}{\cos\phi}$ for a rod sliding along two walls
- **Law of Cosines**: $a^2 = b^2 + c^2 - 2bc\cos A$
- **Law of Sines**: $\dfrac{a}{\sin A} = \dfrac{b}{\sin B} = \dfrac{c}{\sin C} = 2R$
- Proof of the Law of Sines using the **circumcircle** and inscribed angle theorem
- Circumscribed circle uniqueness via perpendicular bisectors
- Inscribed angle theorem: circumferential angle $= \frac{1}{2} \times$ central angle

## Lecture Video

```{=html}
<video controls width="100%" preload="metadata">
  <source src="https://github.com/ymote/learningcalculus/releases/download/v1.0/calculus20251016.mp4" type="video/mp4">
</video>
```

## Key Frames from the Lecture

```{=html}
<div style="display: flex; flex-direction: column; gap: 10px; margin: 1em 0;">
  <img src="../images/calculus20251016_frame1.jpg" style="width:100%; border-radius: 6px;">
  <img src="../images/calculus20251016_frame2.jpg" style="width:100%; border-radius: 6px;">
  <img src="../images/calculus20251016_frame3.jpg" style="width:100%; border-radius: 6px;">
  <img src="../images/calculus20251016_frame4.jpg" style="width:100%; border-radius: 6px;">
</div>
```


## Prerequisites

::: {.callout-note collapse="true"}
## What is the Law of Cosines?

The **Law of Cosines** generalizes the Pythagorean theorem to any triangle. For a triangle with sides $a$, $b$, $c$ and the angle $A$ opposite side $a$:

$$a^2 = b^2 + c^2 - 2bc\cos A$$

When $A = 90°$, we get $\cos A = 0$ and this reduces to $a^2 = b^2 + c^2$, which is just the Pythagorean theorem. This formula determines a missing side given two sides and the included angle, or a missing angle given all three sides.
:::

::: {.callout-note collapse="true"}
## What is vector decomposition?

Any velocity vector can be **decomposed** (split) into two perpendicular components. For example, if you have a velocity $\vec{v}$ and a reference direction, you can write:

$$\vec{v} = v_{\text{radial}} + v_{\text{tangential}}$$

where:

- $v_{\text{radial}} = |\vec{v}|\cos\alpha$ is the component **along** the reference direction
- $v_{\text{tangential}} = |\vec{v}|\sin\alpha$ is the component **perpendicular** to it

Here $\alpha$ is the angle between $\vec{v}$ and the reference direction. This is the standard projection of a vector onto a line using the cosine function.
:::

::: {.callout-note collapse="true"}
## What is a rigid body?

A **rigid body** is an object that cannot stretch, shrink, or deform. A rigid rod of length $L$ means that the distance between its two endpoints is always exactly $L$, no matter how it moves.

This gives us a powerful constraint: the rate at which the distance between the endpoints changes must always be zero:

$$\frac{dL}{dt} = 0$$

In the physics/vector approach, this means the radial components of velocity at both ends (the parts that would stretch or compress the rod) must be equal.
:::

::: {.callout-note collapse="true"}
## What is the circumcircle of a triangle?

The **circumcircle** (or circumscribed circle) is the unique circle that passes through all three vertices of a triangle. Its center is called the **circumcenter**, and its radius is called the **circumradius** $R$.

The circumcenter is found at the intersection of the **perpendicular bisectors** of any two sides. Since a perpendicular bisector of a segment is the set of all points equidistant from the two endpoints, the circumcenter is equidistant from all three vertices.
:::

::: {.callout-note collapse="true"}
## What is the inscribed angle theorem?

The **inscribed angle theorem** states that any angle inscribed in a circle (formed by two chords meeting at a point on the circle) is exactly half the central angle that subtends the same arc:

$$\text{inscribed angle} = \frac{1}{2} \times \text{arc measurement}$$

A key consequence: an angle inscribed in a semicircle (facing a diameter) is always $90°$, because the semicircular arc measures $\pi$ radians and $\frac{\pi}{2} = 90°$.
:::

## The L-Shaped Wall Problem

### Setting Up the Geometry

Consider two walls meeting at an angle of $120°$ at point $O$. A rigid rod $AB$ leans against the walls with:

- $OA = 1$ (distance from the corner to where the rod touches one wall)
- $OB = 2$ (distance from the corner to where the rod touches the other wall)

We first need the length of the rod $AB$ using the **Law of Cosines**:

$$AB^2 = OA^2 + OB^2 - 2 \cdot OA \cdot OB \cdot \cos(120°)$$

$$= 1^2 + 2^2 - 2(1)(2)\cos(120°)$$

Since $\cos 120° = -\frac{1}{2}$:

$$AB^2 = 1 + 4 - 2(1)(2)\!\left(-\frac{1}{2}\right) = 5 + 2 = 7$$

$$\boxed{AB = \sqrt{7}}$$

**Interactive demonstration -- the L-shaped wall configuration:**

::: {.desmos-container}
```{=html}
<div id="lwall" style="width: 100%; height: 400px;"></div>
<script src="https://www.desmos.com/api/v1.9/calculator.js?apiKey=dcb31709b452b1cf9dc26972add0fda6"></script>
<script>
var elt = document.getElementById('lwall');
var calculator = Desmos.GraphingCalculator(elt, {expressions: true, settingsMenu: false});
calculator.setExpression({id: 'wall1', latex: 'y=0 \\left\\{x \\ge 0\\right\\}', color: '#888888', lineWidth: 3});
calculator.setExpression({id: 'wall2', latex: 'y=-\\tan(60\\cdot\\pi/180)\\cdot x \\left\\{x \\le 0\\right\\}', color: '#888888', lineWidth: 3});
calculator.setExpression({id: 'ptO', latex: '(0,0)', color: '#000000', pointSize: 10, label: 'O', showLabel: true});
calculator.setExpression({id: 'ptA', latex: '(1,0)', color: '#2d70b3', pointSize: 10, label: 'A (OA=1)', showLabel: true});
calculator.setExpression({id: 'ptB', latex: '(-2\\cos(\\pi/3), 2\\sin(\\pi/3))', color: '#c74440', pointSize: 10, label: 'B (OB=2)', showLabel: true});
calculator.setExpression({id: 'rod', latex: '(1-t)(1, 0)+t(-2\\cos(\\pi/3), 2\\sin(\\pi/3))', color: '#388c46', lineWidth: 3, parametricDomain: {min: '0', max: '1'}});
calculator.setMathBounds({left: -3, right: 3, bottom: -1, top: 4});
</script>
```
:::

*The green segment is the rigid rod $AB$ with length $\sqrt{7}$. The two gray lines are the walls meeting at $120°$ at the origin $O$.*

### The Physics (Vector Projection) Method

Now suppose point $A$ is pulled along its wall with a horizontal velocity $u = 2$ m/s. We seek the velocity $v$ of point $B$ along the other wall.

The key insight of the **physics method** is:

> For a rigid rod, the component of velocity **along the rod direction** must be the same at both ends.

If the radial (along-the-rod) components were different, the rod would be stretching or compressing -- which is impossible for a rigid body.

### Why the Velocity Must Be Perpendicular to the Rod (Single Fixed Point)

Before tackling the two-moving-endpoints case, consider a simpler scenario: point $A$ is **fixed** and only $B$ can move. What direction can $B$'s velocity point?

- If $B$ had any velocity component **along** the rod, the distance $AB$ would immediately start changing -- the rod would stretch or shrink.
- So $B$'s velocity must be entirely **perpendicular** to the rod. There is no constraint on the magnitude -- $B$ can move fast or slow, it just has to move sideways.
- Geometrically, $B$ moves on a circle centered at $A$, and the instantaneous velocity is tangent to that circle.

### Two Moving Endpoints: Decomposing Velocities

When both $A$ and $B$ are moving, we decompose each velocity into:

- A **radial component** (along the rod $AB$) -- this is the part that would change the rod's length
- A **tangential component** (perpendicular to the rod) -- this just rotates the rod without changing its length

Let $\theta$ be the angle between the rod and the wall at end $A$, and $\phi$ be the angle between the rod and the wall at end $B$.

The velocity $u$ at point $A$ is along the horizontal wall. Its component along the rod direction is:

$$u_{\text{radial}} = u \cos\theta$$

The velocity $v$ at point $B$ is along the other wall. Its component along the rod direction is:

$$v_{\text{radial}} = v \cos\phi$$

### Applying the Rigid Body Constraint

Since the rod cannot change length, the radial components must be equal:

$$u \cos\theta = v \cos\phi$$

Solving for $v$:

::: {.callout-important}
## Key Idea: The Rigid Body Velocity Formula
For a rigid rod that cannot stretch or compress, the components of velocity along the rod at each end must be equal. This yields a direct formula relating the speeds of the two endpoints -- without recourse to calculus.

$$\boxed{v = u \cdot \frac{\cos\theta}{\cos\phi}}$$
:::

No derivatives were required. The velocities are related purely through geometric projection.

**Animated demonstration -- vector projection along a rigid rod:**

```{=html}
<div class="d3-container" id="vec_proj_1016">
  <svg id="vec_proj_svg_1016" width="100%" height="420"></svg>
  <div class="d3-controls">
    <button id="vec_proj_play_1016">&#9654; Play</button>
    <label>Velocity u: <input type="range" id="vec_proj_u_1016" min="0.5" max="5" step="0.1" value="2">
    <span id="vec_proj_u_val_1016">2.0</span> m/s</label>
    <label>Rod angle: <input type="range" id="vec_proj_angle_1016" min="20" max="80" step="1" value="50">
    <span id="vec_proj_angle_val_1016">50</span>&deg;</label>
  </div>
</div>
<script>
(function(){
  var svg = d3.select("#vec_proj_svg_1016");
  var width = 600, height = 420;
  svg.attr("viewBox", "0 0 " + width + " " + height);

  var u = 2, rodAngleDeg = 50;
  var playing = false, animId = null, phase = 0;

  svg.append("defs").html(
    '<marker id="arrowGreen1016" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M0,0 L10,5 L0,10 Z" fill="#388c46"/></marker>' +
    '<marker id="arrowRed1016" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M0,0 L10,5 L0,10 Z" fill="#c74440"/></marker>' +
    '<marker id="arrowBlue1016" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M0,0 L10,5 L0,10 Z" fill="#2d70b3"/></marker>' +
    '<marker id="arrowOrange1016" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M0,0 L10,5 L0,10 Z" fill="#fa7e19"/></marker>'
  );

  var wall1 = svg.append("line").attr("stroke", "#888").attr("stroke-width", 3);
  var wall2 = svg.append("line").attr("stroke", "#888").attr("stroke-width", 3);
  var rod = svg.append("line").attr("stroke", "#388c46").attr("stroke-width", 4).attr("stroke-linecap", "round");
  var dotA = svg.append("circle").attr("r", 7).attr("fill", "#2d70b3");
  var dotB = svg.append("circle").attr("r", 7).attr("fill", "#c74440");
  var velA = svg.append("line").attr("stroke", "#2d70b3").attr("stroke-width", 2.5).attr("marker-end", "url(#arrowBlue1016)");
  var radA = svg.append("line").attr("stroke", "#fa7e19").attr("stroke-width", 2).attr("stroke-dasharray", "5,3").attr("marker-end", "url(#arrowOrange1016)");
  var tanA = svg.append("line").attr("stroke", "#6042a6").attr("stroke-width", 2).attr("stroke-dasharray", "5,3");
  var projBox = svg.append("rect").attr("width", 6).attr("height", 6).attr("fill", "none").attr("stroke", "#fa7e19").attr("stroke-width", 1);

  var labelA = svg.append("text").attr("font-size", "14px").attr("fill", "#2d70b3").attr("font-weight", "bold");
  var labelB = svg.append("text").attr("font-size", "14px").attr("fill", "#c74440").attr("font-weight", "bold");
  var info1 = svg.append("text").attr("x", width / 2).attr("y", height - 40).attr("text-anchor", "middle").attr("font-size", "14px").attr("fill", "#333");
  var info2 = svg.append("text").attr("x", width / 2).attr("y", height - 18).attr("text-anchor", "middle").attr("font-size", "14px").attr("fill", "#fa7e19");

  var Ax, Ay, Bx, By;

  function update() {
    var rodAng = rodAngleDeg * Math.PI / 180;
    var wallAng = 120 * Math.PI / 180;
    var rodLen = 220;
    var cx = 250, cy = 200;

    Ax = cx + rodLen / 2 * Math.cos(-rodAng + Math.PI);
    Ay = cy + rodLen / 2 * Math.sin(-rodAng + Math.PI);
    Bx = cx - rodLen / 2 * Math.cos(-rodAng + Math.PI);
    By = cy - rodLen / 2 * Math.sin(-rodAng + Math.PI);

    wall1.attr("x1", Ax - 120).attr("y1", Ay).attr("x2", Ax + 120).attr("y2", Ay);
    wall2.attr("x1", Bx - 80 * Math.cos(wallAng)).attr("y1", By - 80 * Math.sin(wallAng))
         .attr("x2", Bx + 80 * Math.cos(wallAng)).attr("y2", By + 80 * Math.sin(wallAng));

    rod.attr("x1", Ax).attr("y1", Ay).attr("x2", Bx).attr("y2", By);
    dotA.attr("cx", Ax).attr("cy", Ay);
    dotB.attr("cx", Bx).attr("cy", By);

    var vScale = 40;
    var vx = u * vScale, vy = 0;
    velA.attr("x1", Ax).attr("y1", Ay).attr("x2", Ax + vx).attr("y2", Ay);

    var rodDx = Bx - Ax, rodDy = By - Ay;
    var rodL = Math.sqrt(rodDx * rodDx + rodDy * rodDy);
    var ux = rodDx / rodL, uy = rodDy / rodL;
    var dot = u * vScale * ux;
    var radLen = dot;

    var pulse = 0.5 + 0.5 * Math.sin(phase);
    radA.attr("x1", Ax).attr("y1", Ay).attr("x2", Ax + radLen * ux).attr("y2", Ay + radLen * uy)
      .attr("opacity", 0.6 + 0.4 * pulse);
    tanA.attr("x1", Ax + radLen * ux).attr("y1", Ay + radLen * uy).attr("x2", Ax + vx).attr("y2", Ay);

    labelA.attr("x", Ax - 20).attr("y", Ay + 25).text("A");
    labelB.attr("x", Bx + 10).attr("y", By - 10).text("B");

    var theta = Math.acos(Math.abs(dot) / (u * vScale));
    var uRad = u * Math.cos(theta);
    info1.text("u=" + u.toFixed(1) + " m/s | \u03B8=" + (theta * 180 / Math.PI).toFixed(1) + "\u00B0 | u_radial = u\u00B7cos\u03B8 = " + uRad.toFixed(2) + " m/s");
    info2.text("Orange = radial component (along rod) | Purple = tangential component");
  }

  function animate() {
    if (!playing) return;
    phase += 0.04;
    update();
    animId = requestAnimationFrame(animate);
  }

  document.getElementById("vec_proj_play_1016").addEventListener("click", function() {
    playing = !playing;
    this.textContent = playing ? "\u23F8 Pause" : "\u25B6 Play";
    if (playing) animate();
    else if (animId) cancelAnimationFrame(animId);
  });

  document.getElementById("vec_proj_u_1016").addEventListener("input", function() {
    u = parseFloat(this.value);
    document.getElementById("vec_proj_u_val_1016").textContent = u.toFixed(1);
    update();
  });
  document.getElementById("vec_proj_angle_1016").addEventListener("input", function() {
    rodAngleDeg = parseFloat(this.value);
    document.getElementById("vec_proj_angle_val_1016").textContent = rodAngleDeg;
    update();
  });

  update();
})();
</script>
```

### Solving the Triangle: Finding $\cos\theta$ and $\cos\phi$

We have a triangle $OAB$ with sides $OA = 1$, $OB = 2$, $AB = \sqrt{7}$, and the included angle at $O$ equal to $120°$.

**Finding $\cos\theta$** (angle at vertex $A$, between the rod and the horizontal wall):

Apply the Law of Cosines with $\theta$ as the angle at $A$:

$$OB^2 = OA^2 + AB^2 - 2 \cdot OA \cdot AB \cdot \cos\theta$$

$$4 = 1 + 7 - 2(1)(\sqrt{7})\cos\theta$$

$$\cos\theta = \frac{1 + 7 - 4}{2\sqrt{7}} = \frac{4}{2\sqrt{7}} = \frac{2}{\sqrt{7}}$$

**Finding $\cos\phi$** (angle at vertex $B$, between the rod and the other wall):

$$OA^2 = OB^2 + AB^2 - 2 \cdot OB \cdot AB \cdot \cos\phi$$

$$1 = 4 + 7 - 2(2)(\sqrt{7})\cos\phi$$

$$\cos\phi = \frac{4 + 7 - 1}{4\sqrt{7}} = \frac{10}{4\sqrt{7}} = \frac{5}{2\sqrt{7}}$$

### Computing the Final Answer

$$v = u \cdot \frac{\cos\theta}{\cos\phi} = 2 \cdot \frac{\;\dfrac{2}{\sqrt{7}}\;}{\;\dfrac{5}{2\sqrt{7}}\;} = 2 \cdot \frac{2}{\sqrt{7}} \cdot \frac{2\sqrt{7}}{5} = 2 \cdot \frac{4}{5} = \frac{8}{5}$$

$$\boxed{v = \frac{8}{5} = 1.6 \text{ m/s}}$$

Notice how the $\sqrt{7}$ cancels out cleanly -- there is no need to rationalize intermediate expressions.

**Animated demonstration -- approaching velocity decomposed into radial and tangential components:**

```{=html}
<div class="d3-container" id="approach_vel_1016">
  <svg id="approach_vel_svg_1016" width="100%" height="420"></svg>
  <div class="d3-controls">
    <button id="approach_vel_play_1016">&#9654; Play</button>
    <label>Speed: <input type="range" id="approach_vel_speed_1016" min="0.5" max="4" step="0.1" value="2">
    <span id="approach_vel_speed_val_1016">2.0</span> m/s</label>
  </div>
</div>
<script>
(function(){
  var svg = d3.select("#approach_vel_svg_1016");
  var width = 600, height = 420;
  svg.attr("viewBox", "0 0 " + width + " " + height);

  var speed = 2, playing = false, animId = null;
  var angle = 0;
  var fixedX = 420, fixedY = 200;
  var orbitR = 180;

  svg.append("defs").html(
    '<marker id="arrowMov1016" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M0,0 L10,5 L0,10 Z" fill="#2d70b3"/></marker>' +
    '<marker id="arrowRad1016" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M0,0 L10,5 L0,10 Z" fill="#c74440"/></marker>' +
    '<marker id="arrowTan1016" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M0,0 L10,5 L0,10 Z" fill="#388c46"/></marker>'
  );

  var fixedDot = svg.append("circle").attr("cx", fixedX).attr("cy", fixedY).attr("r", 8).attr("fill", "#c74440");
  svg.append("text").attr("x", fixedX + 12).attr("y", fixedY + 5).text("Fixed point").attr("font-size", "13px").attr("fill", "#c74440");

  var distLine = svg.append("line").attr("stroke", "#aaa").attr("stroke-width", 1).attr("stroke-dasharray", "4,4");
  var movDot = svg.append("circle").attr("r", 8).attr("fill", "#2d70b3");
  var velArrow = svg.append("line").attr("stroke", "#2d70b3").attr("stroke-width", 2.5).attr("marker-end", "url(#arrowMov1016)");
  var radArrow = svg.append("line").attr("stroke", "#c74440").attr("stroke-width", 2).attr("stroke-dasharray", "5,3").attr("marker-end", "url(#arrowRad1016)");
  var tanArrow = svg.append("line").attr("stroke", "#388c46").attr("stroke-width", 2).attr("stroke-dasharray", "5,3").attr("marker-end", "url(#arrowTan1016)");

  var trail = svg.append("path").attr("fill", "none").attr("stroke", "#2d70b3").attr("stroke-width", 1).attr("opacity", 0.3);
  var trailPoints = [];

  var info1 = svg.append("text").attr("x", width / 2).attr("y", height - 40).attr("text-anchor", "middle").attr("font-size", "13px").attr("fill", "#333");
  var info2 = svg.append("text").attr("x", width / 2).attr("y", height - 18).attr("text-anchor", "middle").attr("font-size", "13px").attr("fill", "#333");

  function update() {
    var mx = 160 + orbitR * Math.cos(angle);
    var my = fixedY + 80 * Math.sin(angle * 1.7);

    movDot.attr("cx", mx).attr("cy", my);
    distLine.attr("x1", mx).attr("y1", my).attr("x2", fixedX).attr("y2", fixedY);

    var velAngle = angle + Math.PI / 2 + 0.3 * Math.sin(angle);
    var vScale = 35;
    var vx = speed * Math.cos(velAngle) * vScale;
    var vy = speed * Math.sin(velAngle) * vScale;
    velArrow.attr("x1", mx).attr("y1", my).attr("x2", mx + vx).attr("y2", my + vy);

    var dx = fixedX - mx, dy = fixedY - my;
    var dist = Math.sqrt(dx * dx + dy * dy);
    var ux = dx / dist, uy = dy / dist;
    var radComp = vx * ux + vy * uy;
    var radVx = radComp * ux, radVy = radComp * uy;
    var tanVx = vx - radVx, tanVy = vy - radVy;

    radArrow.attr("x1", mx).attr("y1", my).attr("x2", mx + radVx).attr("y2", my + radVy);
    tanArrow.attr("x1", mx + radVx).attr("y1", my + radVy).attr("x2", mx + vx).attr("y2", my + vy);

    var vRadial = radComp / vScale;
    var approaching = vRadial > 0 ? "approaching" : "receding";
    info1.text("Blue = velocity | Red = radial (changes distance) | Green = tangential (rotation only)");
    info2.text("v_radial = " + vRadial.toFixed(2) + " m/s (" + approaching + ") | distance = " + (dist / 2).toFixed(0) + " units");
  }

  function animate() {
    if (!playing) return;
    angle += speed * 0.008;
    if (angle > 2 * Math.PI) angle -= 2 * Math.PI;
    update();
    animId = requestAnimationFrame(animate);
  }

  document.getElementById("approach_vel_play_1016").addEventListener("click", function() {
    playing = !playing;
    this.textContent = playing ? "\u23F8 Pause" : "\u25B6 Play";
    if (playing) animate();
    else if (animId) cancelAnimationFrame(animId);
  });

  document.getElementById("approach_vel_speed_1016").addEventListener("input", function() {
    speed = parseFloat(this.value);
    document.getElementById("approach_vel_speed_val_1016").textContent = speed.toFixed(1);
    update();
  });

  update();
})();
</script>
```

## Proof of the Law of Sines

### Uniqueness of the Circumcircle

Every triangle has a unique **circumcircle** passing through all three vertices. The circumcenter $O$ must lie on:

1. The **perpendicular bisector** of $AB$ (equidistant from $A$ and $B$)
2. The **perpendicular bisector** of $AC$ (equidistant from $A$ and $C$)

These two lines meet at exactly one point. By transitivity, that point is also equidistant from $B$ and $C$, confirming it lies on the perpendicular bisector of $BC$ as well. This unique point is the circumcenter, and its distance to any vertex is the circumradius $R$.

### The Inscribed Angle Theorem

All inscribed angles subtending the same arc are equal, and each equals half the central angle:

$$\text{Inscribed angle} = \frac{1}{2} \times \text{central angle (arc measurement)}$$

A crucial special case: any angle inscribed in a semicircle equals $90°$.

### Proving $\dfrac{a}{\sin A} = 2R$

Draw the diameter $BD$ through vertex $B$. Since $\angle BCD$ is inscribed in a semicircle:

$$\angle BCD = 90°$$

By the inscribed angle theorem, $\angle BDC = \angle A$ (both subtend arc $BC$).

In the right triangle $BDC$ with hypotenuse $BD = 2R$:

$$\sin(\angle BDC) = \frac{BC}{BD} = \frac{a}{2R}$$

Since $\angle BDC = \angle A$:

$$\sin A = \frac{a}{2R} \implies \frac{a}{\sin A} = 2R$$

By the same argument applied to the other vertices:

::: {.callout-important}
## Key Idea: The Law of Sines
In any triangle, the ratio of a side to the sine of its opposite angle is the same for all three sides -- and that common value equals the diameter of the circumscribed circle.

$$\boxed{\frac{a}{\sin A} = \frac{b}{\sin B} = \frac{c}{\sin C} = 2R}$$
:::

**Interactive demonstration -- the circumcircle and the inscribed angle theorem:**

::: {.desmos-container}
```{=html}
<div id="circumcircle" style="width: 100%; height: 400px;"></div>
<script>
var elt2 = document.getElementById('circumcircle');
var calc2 = Desmos.GraphingCalculator(elt2, {expressions: true, settingsMenu: false});
calc2.setExpression({id: 'circle', latex: 'x^2+y^2=4', color: '#aaaaaa', lineWidth: 2});
calc2.setExpression({id: 'pA', latex: '(2\\cos(2.5), 2\\sin(2.5))', color: '#2d70b3', pointSize: 10, label: 'A', showLabel: true});
calc2.setExpression({id: 'pB', latex: '(2\\cos(0.3), 2\\sin(0.3))', color: '#c74440', pointSize: 10, label: 'B', showLabel: true});
calc2.setExpression({id: 'pC', latex: '(2\\cos(4.5), 2\\sin(4.5))', color: '#388c46', pointSize: 10, label: 'C', showLabel: true});
calc2.setExpression({id: 'sAB', latex: '(1-t)(2\\cos(2.5), 2\\sin(2.5))+t(2\\cos(0.3), 2\\sin(0.3))', color: '#2d70b3', lineWidth: 2, parametricDomain: {min: '0', max: '1'}});
calc2.setExpression({id: 'sBC', latex: '(1-t)(2\\cos(0.3), 2\\sin(0.3))+t(2\\cos(4.5), 2\\sin(4.5))', color: '#c74440', lineWidth: 2, parametricDomain: {min: '0', max: '1'}});
calc2.setExpression({id: 'sCA', latex: '(1-t)(2\\cos(4.5), 2\\sin(4.5))+t(2\\cos(2.5), 2\\sin(2.5))', color: '#388c46', lineWidth: 2, parametricDomain: {min: '0', max: '1'}});
calc2.setExpression({id: 'diam', latex: '(1-t)(2\\cos(0.3), 2\\sin(0.3))+t(-2\\cos(0.3), -2\\sin(0.3))', color: '#fa7e19', lineWidth: 2, lineStyle: 'DASHED', parametricDomain: {min: '0', max: '1'}});
calc2.setExpression({id: 'pD', latex: '(-2\\cos(0.3), -2\\sin(0.3))', color: '#fa7e19', pointSize: 8, label: 'D', showLabel: true});
calc2.setExpression({id: 'sDC', latex: '(1-t)(-2\\cos(0.3), -2\\sin(0.3))+t(2\\cos(4.5), 2\\sin(4.5))', color: '#fa7e19', lineWidth: 1, lineStyle: 'DASHED', parametricDomain: {min: '0', max: '1'}});
calc2.setMathBounds({left: -4, right: 4, bottom: -3.5, top: 3.5});
</script>
```
:::

*Triangle $ABC$ is inscribed in a circle of radius $R = 2$. The orange dashed line is the diameter $BD$. Triangle $BDC$ is a right triangle (angle at $C$ is $90°$), and angle $D$ equals angle $A$ by the inscribed angle theorem.*

## Rigid Rod on a Circle (Homework Problem)

The following is the second example from class, assigned as homework:

A rod $AB$ has length equal to one side of a regular hexagon inscribed in a given circle (so the arc $AB$ subtends $60°$, meaning $AB$ equals the radius $r$).

Point $B$ moves **radially outward** from the center at $v = 2$ m/s. Point $A$ is constrained to move **along the circle** (tangentially). Find the speed of $A$.

**Hints:**

1. Since arc $AB$ subtends $60°$ at the center, the triangle $OAB$ (where $O$ is the center) is **equilateral** -- all sides equal $r$ and all angles equal $60°$
2. The velocity of $A$ must be along the **tangent line** to the circle at $A$ (perpendicular to the radius $OA$)
3. The velocity of $B$ is along the **radial direction** from the center through $B$
4. Decompose both velocities along the rod direction $AB$
5. Set the radial components equal (rigid body constraint): $v_A \cos\alpha = v_B \cos\beta$
6. Use the equilateral triangle geometry to find $\alpha$ and $\beta$

The method is identical to the L-shaped wall problem -- only the directions of the constrained motions have changed.

## Cheat Sheet

::: {.key-formula}
| Formula | What it means |
|---|---|
| $a^2 = b^2 + c^2 - 2bc\cos A$ | **Law of Cosines** -- find a side or angle in any triangle |
| $\dfrac{a}{\sin A} = \dfrac{b}{\sin B} = \dfrac{c}{\sin C} = 2R$ | **Law of Sines** -- relate sides to opposite angles and the circumradius |
| $\cos 120° = -\dfrac{1}{2}$ | Key value used in the L-wall problem |
| $u\cos\theta = v\cos\phi$ | **Rigid body constraint** -- radial velocity components must match |
| $v = u\dfrac{\cos\theta}{\cos\phi}$ | Solving for the unknown velocity of the other endpoint |

### The Vector Projection Method (Step by Step)

1. Identify the **rigid rod** connecting two moving points
2. Determine the **direction** each point is constrained to move
3. **Decompose** each velocity into radial (along the rod) and tangential (perpendicular to the rod) components
4. Set the **radial components equal**: $v_A^{\text{radial}} = v_B^{\text{radial}}$
5. Solve for the unknown speed

### Key Geometry Facts

$$\text{Inscribed angle} = \frac{1}{2} \times \text{arc (central angle)}$$

$$\text{Angle inscribed in a semicircle} = 90°$$

$$\text{Circumcenter} = \text{intersection of perpendicular bisectors}$$

### When to Use Each Method

| Method | Use when... |
|---|---|
| **Vector projection (physics)** | Two points connected by a rigid rod, each constrained to a known path |
| **Calculus (related rates)** | General relationships between changing quantities, not limited to rigid rods |
:::
