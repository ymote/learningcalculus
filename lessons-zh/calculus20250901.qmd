...
最优化是关于找到**最佳**答案——最短路径、最低成本设计、最快路线。
...
**导数** $f'(x)$ 告诉你**变化率**——$f(x)$ 在任意点 $x$ 处增加或减少的速度。
...
当 $a = \sqrt{P}$ 时，我们得到 $b = \frac{P}{\sqrt{P}} = \sqrt{P} = a$。所以和 $a + b$ 在 $a = b$ 时最小——这正是 AM 等于 GM 的时候！
...
有一个优美的几何捷径。在曲线上的最近点处，从 $P$ 到曲线的连线必须**垂直于切线**。
...
这告诉我们**切线方向**：在 $x$ 方向移动 $dx$ 会在 $y$ 方向引起 $dy = 2x \, dx$。所以切向量是：
