...
本节课将推导正切和正割的导数——这两个三角函数可以在有限区间内趋向无穷大。我们将用三种不同的方法证明同一个结论：代数方法、单位圆上的几何方法以及复指数方法，三条路径最终都给出相同的结果。
...
- **建筑**：屋顶的坡度是一个正切比，导数反映坡度对角度变化的敏感程度
- **光学**：透镜以正切描述的角度折射光线——理解角度变化的速度是设计相机和望远镜的关键
- **过山车**：在陡峭段，轨道角度大且正切增长快——其导数（sec²）告诉工程师斜率增加的速度有多快

在已知 sin 和 cos 的导数的基础上，tan 和 sec 的导数可以自然地推导出来。本节课将用三种不同的方法对此加以验证。
:::
...
复指数的优势在于 $e^{i\theta}$ 的导数就是 $ie^{i\theta}$，无需额外记忆符号变换规则。
:::
...
**交互演示——观察 $\tan\theta$ 及其导数 $\sec^2\theta$：**
...
*拖动滑块 $a$。绿色虚线是 $\tan x$（蓝色）的切线，其斜率始终等于 $\sec^2(a)$（红色虚线曲线）。可以观察到斜率始终不小于 1，即 $\tan x$ 的增长率单调不减。*

### 动画演示：切线沿 tan(x) 滑动，斜率 = sec²(x)
...
因此 $\frac{d(\tan\theta)}{d\theta} = \sec^2\theta$，与代数方法的结果一致，此处直接来自几何推导。

**交互演示——观察 $\tan\theta$ 和 $\sec\theta$ 的单位圆构造：**
...
*拖动 $\theta$，观察正切高度（红色线段）和正割长度（紫色标签）的变化。可以观察到当 $\theta$ 趋近 $\pm\frac{\pi}{2}$ 时，$\tan\theta$ 增长极为迅速。*

### 动画演示：sec(x) 及其导数 sec(x)tan(x) 的滑动标记
...
::: {.callout-important}
## 核心要点：正割的导数
正割的导数是正割乘以正切。可以用单位圆上的相似三角形进行几何证明，也可以用链式法则对 $(\cos\theta)^{-1}$ 进行代数证明。
...
在处理形如 $\frac{e^{i\theta} - e^{-i\theta}}{e^{i\theta} + e^{-i\theta}}$ 的表达式时，可以将分子和分母同除以 $e^{-i\theta}$ 来化简：

$$\frac{e^{i\theta} - e^{-i\theta}}{e^{i\theta} + e^{-i\theta}} = \frac{e^{2i\theta} - 1}{e^{2i\theta} + 1}$$

也可以令 $w = e^{i\theta}$，将所有内容转化为简洁的多项式形式的分数：
...
这种"无穷小代入"方法较为直观，可以清晰地看到导数的每一部分从何而来。

::: {.callout-tip collapse="true"}
## 为什么可以忽略 $(d\theta)^2$？

无穷小量 $d\theta$ 的平方在取极限后趋于零。更准确地说，当在 $\frac{\Delta f}{d\theta}$ 中令 $d\theta \to 0$ 时，分子中含 $(d\theta)^2$ 的任何项在除以 $d\theta$ 后贡献为 $0$。因此只需保留一阶项，其余各项均可忽略。
:::

**交互演示——比较 $\tan(x)$ 与其在某点的线性近似：**
