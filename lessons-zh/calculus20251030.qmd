...
你从小学起就知道圆的周长公式 $C = 2\pi r$——但你见过它为什么成立的真正证明吗？在本课中，你将使用参数方程描述一个点在圆上的运动，通过求导计算它的速率，然后用微积分从零开始证明周长公式。在此过程中，你还将学习如何通过"反向"微分从速度恢复物体的位置。
...
- **GPS 导航**：你的手机将你的位置追踪为独立的 $x(t)$ 和 $y(t)$ 坐标，以在地图上绘制你的路线
...
一辆以 60 英里/时向北行驶的汽车，其速度为 60 英里/时向北，速率为 60 英里/时。如果它掉头以 60 英里/时向南行驶，速度改变了（方向不同），但速率不变。
:::

::: {.callout-note collapse="true"}
## 什么是链式法则？

当你有一个函数嵌套在另一个函数内时，**链式法则**告诉你如何求导：
...
- **位移**是从起点到终点的直线向量。如果你向东走 3 个街区再向西走 3 个街区，你的位移为零。
- **路程**是你实际走过的路径总长度。在上面的例子中，路程是 6 个街区。
...
可以这样理解：如果你以恒定速率 $2r$ 行驶时间 $\pi$，你走过的距离就是 $2r \times \pi$。它就是恒定速度图形下方矩形的面积。
...
对于完整的圆，位移是 $\vec{0}$（你回到了起点），但路程是 $2\pi r$。
