...
科学家如何测量曲线图形的面积？今天你将看到如何把一个圆切成无限薄的条带，让微积分计算出仅靠几何学难以处理的面积。你还将学习如何"撤销"导数——这项技能叫做反微分——并发现当你处理圆时，反三角函数（如反正弦）会在意想不到的地方出现。在此过程中，你将证明抛出的球的轨迹是一条抛物线。

::: {.callout-tip collapse="true"}
## 为什么反导数和积分很重要

积分是微积分的第二大支柱——它让你把无数个微小部分加在一起以得到总量：

- **医学**：医生通过对吸收速率积分来计算你的身体在一段时间内吸收的药物总量——剂量计算错误可能是危险的
...
但 $F(x) = x^2 + 7$ 同样成立！加上任何常数的反导数仍然是反导数，因为常数在求导时消失。这就是为什么我们在末尾写 $+C$（一个任意常数）。
:::
...
例如，如果你知道 $\frac{d}{dt}(t^2) = 2t$，那么 $2t$ 的反导数就是 $t^2 + C$。等价地，$t$ 的反导数是 $\frac{t^2}{2} + C$。
:::
...
::: {.callout-important}
## 核心要点：水平抛射的轨迹是抛物线
通过从速度求位置（反微分）然后消去时间，你可以证明水平抛出的球描绘出抛物线路径。重力控制曲线的陡峭程度，而抛出速度控制曲线的平坦程度。
...
*拖动 $g$ 来增大重力（抛物线变陡），或增大 $u$ 来更快地抛出（抛物线变平）。球始终沿抛物线路径运动！*
...
关键观察：**当你看到像 $\frac{1}{\sqrt{1-x^2}}$ 这样的根式并需要其反导数时，想到反三角函数。当你看到 $\frac{1}{x}$ 时，想到对数。** 导数"离开"了函数族，所以反导数"回到"了函数族。
...
::: {.callout-important}
## 核心要点：反正弦的导数
$\arcsin x$ 的导数产生一个根式表达式——它"离开"了反三角函数族。这意味着当你看到 $\frac{1}{\sqrt{1-x^2}}$ 并需要其反导数时，答案就是 $\arcsin x$。这正是让我们能够对圆形区域积分的关键联系。
...
*将 $a$ 在 $-1$ 和 $1$ 之间拖动。$\arcsin x$（蓝色）的绿色切线的斜率为 $\frac{1}{\sqrt{1-a^2}}$（红色虚线）。注意在 $x = \pm 1$ 附近斜率趋向无穷——曲线在那里变成竖直的！*
